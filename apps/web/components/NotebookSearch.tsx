 ...
 * "Kelime" modu kota harcamaz (aninda); "Anlam" modu 1 gomme istegi harcar.
 */
import { useEffect, useRef, useState } from "react";
import { useRouter } from "next/navigation";
import { api } from "@/lib/api";
import { Search, X, Loader2, FileText, ExternalLink, Type, Brain } from "lucide-react";
...
export default function NotebookSearch({ collectionId, readyCount }: { collectionId: string; readyCount: number }) {
  ...
  const [mode, setMode] = useState<"hybrid" | "text" | "meaning">("hybrid");
  ...
  // Yazarken 350 ms bekle; kelime modunda aninda, anlam modunda gomme kotasini bosa yakmamak icin
  useEffect(() => {
    ...
    <div className="mb-4 rounded-2xl border bg-surface p-3">
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative min-w-[220px] flex-1">
          ...
          {q && (
            <button onClick={() => { setQ(""); setRes(null); }} aria-label="Temizle"
                    className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md p-1 text-text-secondary hover:bg-surface-muted">
              {busy ? <Loader2 size={14} className="animate-spin" /> : <X size={14} />}
            </button>
          )}
        </div>
        <div className="flex items-center gap-0.5 rounded-xl border p-0.5" title="Kelime: birebir geçen yerler, kota harcamaz · Anlam: yakın kavramlar da">
          {([["hybrid", "İkisi", null], ["text", "Kelime", Type], ["meaning", "Anlam", Brain]] as const).map(([k, l, I]) => (
            <button key={k} onClick={() => setMode(k)}
                    className={cx("flex items-center gap-1 rounded-lg px-2.5 py-1 text-xs",
      ...
      {res && (
        <div className="mt-3">
          <p className="mb-2 text-xs text-text-secondary">
            {res.total === 0
              ? <>"<b>{res.query}</b>" için sonuç yok{res.mode === "text" ? " — Anlam modunu dene" : ""}.</>
              : <><b>{res.groups.length}</b> kaynakta <b>{res.total}</b> yer · tıkla, PDF'te o sayfa açılsın</>}
          </p>
              ...
              <div key={g.document_id} className="rounded-xl border bg-surface-muted/40">
                ...
                <div className="divide-y border-t">
                  {g.hits.map((h, i) => (
                    <button key={i} onClick={() => open(g.document_id, h.page)}
                      ...
                      <span className={cx("mt-0.5 shrink-0 rounded-md px-1.5 py-0.5 text-[11px] font-medium",
                        h.how === "meaning" ? "bg-accent-purple/10 text-accent-purple" : "bg-accent-amber/15 text-accent-amber")}>
                        s.{h.page ?? "?"}
                      </span>
                      <span className="min-w-0 flex-1 text-xs leading-relaxed text-text-secondary">
                        {h.section && <span className="mr-1 text-[10px] uppercase tracking-wide opacity-70">{h.section} ·</span>}
                  ...
                  {g.count > g.hits.length && (
                    <p className="px-3 py-1.5 text-[11px] text-text-secondary">+{g.count - g.hits.length} yer daha bu belgede</p>
                  )}
                </div>
              </div>
            ))}
