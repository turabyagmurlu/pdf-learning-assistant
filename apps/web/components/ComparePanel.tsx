...
const STANCE: Record<string, { t: string; tone: BadgeTone }> = {
  destekliyor: { t: "Destekliyor", tone: "success" },
  karsi: { t: "Karşı", tone: "danger" },
  karma: { t: "Karma", tone: "warning" },
  notr: { t: "Nötr / betimleyici", tone: "neutral" },
};
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="flex gap-2">
        ...
        <Button variant="primary" size="lg" onClick={() => run()} disabled={busy || topic.trim().length < 3} title={costTitle(1)} cost={1}>
      ...
      {!res && !busy && hints.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {hints.slice(0, 8).map((h) => (
            <button key={h} onClick={() => run(h)} title={costTitle(1)} className="min-h-[36px] rounded-full border bg-surface px-3 py-1 text-xs text-text-secondary hover:border-border-strong hover:text-text-primary">{h} <Cost n={1} /></button>
          ))}
        </div>
      )}
        ...
        <div className="mt-5 space-y-5">
          <div className="rounded-2xl border bg-surface p-4">
            <div className="flex items-center gap-2">
              ...
              {res.cached && <Badge tone="success">Kayıtlı sonuç · çağrı yapılmadı</Badge>}
            ...
            <p className="mt-2 font-reading">{res.summary}</p>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-danger/30 bg-danger-bg/40 p-4">
              <p className="flex items-center gap-1.5 text-sm font-medium text-danger"><AlertTriangle size={15} /> Çelişkiler ({res.conflicts.length})</p>
              {!res.conflicts.length ? <p className="mt-2 text-sm text-text-secondary">Kaynaklar bu konuda birbirine ters düşmüyor.</p> : res.conflicts.map((c, i) => (
                <div key={i} className="mt-3 rounded-xl border bg-surface p-3 text-sm">
                  ...
                  <ul className="mt-1.5 space-y-1">
                    {c.sides.map((s, j) => (
                      <li key={j} className="text-sm"><button onClick={() => open(s.document_id)} className="font-medium text-accent-purple hover:underline">{s.title}</button>: {s.claim}</li>
                    ))}
                  </ul>
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-success/30 bg-success-bg/40 p-4">
                <p className="flex items-center gap-1.5 text-sm font-medium text-success"><CheckCircle2 size={15} /> Uyuştukları noktalar</p>
