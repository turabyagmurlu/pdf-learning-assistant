"use client";
/**
 * Cevap metnini markdown olarak (kalın, liste, başlık) ve [K1], [K2, K3] gibi atıf
 * etiketlerini tıklanabilir rozetlere çevirerek gösterir. Her rozet ilgili kaynağın
 * sayfasına götürür: iddia → kanıt tek tıkla.
 *
 * Ön izleme: farede üzerine gelince, klavyede odaklanınca açılır (Esc kapatır).
 * Dokunmatikte ilk dokunuş ön izlemeyi alttan açılan küçük tabakada gösterir;
 * ikinci dokunuş ya da "Kaynakta aç" kaynağa gider.
 *
 * Markdown işleyici: lib/markdown.ts + components/Markdown.tsx (HTML üretmez).
 */
...
export default function CitedText({ text, sources, onCite, className }: {
  ...
  /** Tek bir [K#] rozeti (Markdown bileşeni her numara için çağırır; sıra render sırasına göre sabittir). */
  const badge = (n: number) => {
    const src = sources[n - 1];
    const loc = citeLoc(src);
    const key = "c" + (seq.current++) + "-" + n;
    ...
    return (
      <span className="relative inline-block"
        ...
        <button type="button"
                onClick={(e) => {
                  e.stopPropagation();
                  // Dokunmatik: ilk dokunuş ön izleme, ikincisi kaynağa gider
          ...
          <>
            ...
            <span id={popId} role="dialog" aria-label={`K${n} alıntı önizlemesi`}
                  ...
                  className={open?.sheet
                    ? "fixed inset-x-0 bottom-0 z-[70] block rounded-t-2xl border bg-surface p-4 pb-[max(1rem,env(safe-area-inset-bottom))] text-left font-body text-sm shadow-medium"
                    : "absolute left-1/2 top-full z-40 mt-1.5 block w-72 -translate-x-1/2 rounded-xl border bg-surface p-3 text-left font-body text-sm shadow-medium"}>
  ...
  return <Markdown text={text} cite={badge} className={className || "text-sm leading-relaxed"} />;
}
